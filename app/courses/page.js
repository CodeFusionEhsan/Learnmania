      ...
      <div className="container mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {courses.map(course => (
          <div key={course._id} className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <img src={`/images/${course.image}`} alt={course.ct} className="h-40 w-full object-cover rounded-t-lg" />
            ...
            <a href={`/course/${course._id}`} className="mt-4 block bg-blue-500 hover:bg-blue-600 text-center text-white font-bold py-2 px-4 rounded transition duration-200">
              Check Out Course
            </a>
          </div>
        ))}
